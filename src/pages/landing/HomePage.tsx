...
function FinalCTA() {
  ...
  const inputCls = "w-full h-12 rounded-full bg-[#F2F0EB] px-5 text-right placeholder:text-[#9CA3AF] text-sm outline-none focus:ring-2 focus:ring-[var(--gold)]/50 border-0";

  return (
    <section id="contact" className="relative py-20 lg:py-28 bg-[var(--cream)] scroll-mt-24">
      <div className="container mx-auto px-6 lg:px-12">
        <div className="text-center mb-12">
          <h2 className="font-extrabold text-[var(--navy-dark)] leading-tight mb-6" style={{ fontSize: "clamp(2.5rem, 6vw, 5rem)" }}>
            יצירת קשר
          </h2>
          <p className="text-lg md:text-xl text-[var(--ink)]/80 font-light max-w-3xl mx-auto leading-relaxed">
        ...
        <div className="max-w-6xl mx-auto bg-white rounded-[30px] shadow-[0_20px_60px_-20px_rgba(11,31,74,0.15)] p-6 md:p-14">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            <div className="order-2 lg:order-1">
              <img src={contactIllustration} alt="יצירת קשר" className="w-full h-auto object-contain rounded-3xl" loading="lazy" width={1400} height={1050} />
            </div>

            <div className="order-1 lg:order-2">
              {sent ? (
                <div className="bg-[var(--gold)]/10 border border-[var(--gold)]/30 rounded-2xl p-10 text-center">
                  <CheckCircle2 className="w-14 h-14 text-[var(--gold)] mx-auto mb-5" />
                  <h3 className="text-2xl font-bold text-[var(--navy-dark)] mb-2">תודה רבה! 🎉</h3>
                  <p className="text-[var(--ink)]/70">קיבלנו את הפנייה ונחזור אליכם בהקדם.</p>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    <label className="block text-lg font-semibold text-[#374151] mb-2">שם מלא</label>
                    <input name="full_name" value={form.full_name} onChange={handleChange} placeholder="הכנס את שמך" maxLength={100} required className={inputCls} />
                  </div>
                  <div>
                    <label className="block text-lg font-semibold text-[#374151] mb-2">אימייל</label>
                    <input name="email" type="email" value={form.email} onChange={handleChange} placeholder="הכנס אימייל" maxLength={255} className={inputCls} />
                  </div>
                  <div>
                    <label className="block text-lg font-semibold text-[#374151] mb-2">טלפון</label>
                    <input name="phone" value={form.phone} onChange={handleChange} placeholder="הכנס מספר טלפון" maxLength={15} required className={inputCls} />
                  </div>
                  <div>
                    <label className="block text-lg font-semibold text-[#374151] mb-2">הודעה</label>
                    <textarea name="message" value={form.message} onChange={handleChange} placeholder="כתוב לנו מה אתה צריך...." maxLength={1000} className="w-full min-h-[140px] rounded-3xl bg-[#F2F0EB] p-4 text-right placeholder:text-[#9CA3AF] text-sm outline-none focus:ring-2 focus:ring-[var(--gold)]/50 border-0 resize-none" />
                  </div>
                  <button type="submit" disabled={loading} className="w-full h-[52px] rounded-xl bg-[var(--gold)] hover:bg-[var(--gold-light)] transition-colors text-white text-xl font-bold disabled:opacity-60">
                    {loading ? "שולח..." : "שליחה ←"}
                  </button>
                </form>
              )}
